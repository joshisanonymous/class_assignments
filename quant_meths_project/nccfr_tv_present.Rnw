...
\begin{document}
  ...
  \section{Introduction}
    ...
    \begin{frame}{Objective}
      \begin{block}{}
        To identify if different social groups in the Nijmegen Corpus of Casual French (NCCFr) \citep{torreira_nijmegen_2010} have the same patterns of T-V usage
      \end{block}
    \end{frame}
  ...
  \section{Results}
    \begin{frame}{Hypothesis Tests}
      ...
      \begin{block}<2->{}
        \begin{table}
          \caption{Results for each explanatory variable separately}
          \label{tab:coef_tests}
          \begin{tabular}{l @{ } r @{ } r}
            Variable (Reference) & Coefficient & $P$-value ($<$) \\
            \hline
            \hline
            Gender (Female) & \Sexpr{round(tv.lrm$coefficients["GENDER=male"], 3)} & \Sexpr{round_any(tv.lrm.anova["GENDER","P"], 0.001, f = ceiling)} \\
            Region (Not Paris) & \Sexpr{round(tv.lrm$coefficients["REGION=paris"], 3)} & \Sexpr{round_any(tv.lrm.anova["REGION","P"], 0.001, f = ceiling)} \\
  ...
  \section{Conclusion}
    \begin{frame}{Conclusion}
      \begin{block}{}
        ...
        \begin{itemize}
          \item A model that includes only gender has a coefficient of \Sexpr{round(tv.lrm.final$coefficients["GENDER=male"], 3)}
          \item where \emph{tu} is the reference level of T-V usage and
          \item female is the reference level of gender
        \end{itemize}
      \end{block}
      \begin{block}{}
        \begin{table}
          \caption{Proportion of \emph{tu} vs \emph{vous} by gender}
