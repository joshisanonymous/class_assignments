\documentclass{article}
  ...
  \title{Verb forms used to express the habitual present by Louisiana Creole speakers}
...
\begin{document}
  ...
  \section{Results}
    ...
    \begin{figure}[tbhp]
      ...
      \centering
      <<freq_race_speaker, fig.height = 6, fig.width = 8>>=
      @
    \end{figure}
  ...
  \section{Discussion}
    ...
    % first question: do races differ in their speech
      % they do, with caveats
        % whites prefer short forms, but almost all from one speaker
        % creoles prefer long forms, but almost all from one speaker
        % blacks seem to have no pattern
      % why aren't the results more conclusive?
        % not enough speakers in each category, but that's the smaller issue
        % perhaps the categories were not salient enough here
          % several speakers described growing up spending time with other races
            % gives quotes
          % milroy showed long ago the importance of who we interact with
      % why did klingler conclude that there was a race difference?
        % it's not clear, but perhaps he was expecting to find it
          % long forms certainly were more common with blacks, even if they weren't to the exclusion of short forms
          % fact that he saw these long forms being used by black and none by whites may have led him to overlook that black also use short forms regularly
          % no reason to believe that there was any intention to reify racializations, but even careful researchers can be prone to this in a racialized society
    % second question: what does this mean about race relations in pointe coupee?
      % they may simply be more complicated than was expected
        % there were several categories described and terms used but only 3 classifications
        % even the idea of cajun as white was contradicted by (someone) saying they were darker skinned than creoles
